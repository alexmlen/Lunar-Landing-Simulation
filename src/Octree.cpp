...
void Octree::getChildren(const TreeNode & node) {
	if (node.children.size() != 0) {
		for (int i = 0; i < node.children.size(); i++) {
...
ofVec3f Octree::altitude(const Ray &ray) {
	vector<TreeNode> leaves{};
	getChildren(root);
	for (int i = 0; i < leaves.size(); i++) {
		bool hit = leaves[i].box.intersect(ray, 0, 10000);
		if (hit) {
			ofVec3f vert = mesh.getVertex(i);
			return vert;
		}
	}
	return ofVec3f(0, std::numeric_limits<float>::infinity(), 0);
}

ofVec3f Octree::intersect(const Ray &ray, const TreeNode & node) {
	if (node.children.size() > 0) {
		for (int i = 0; i < node.children.size(); i++) {
			ofVec3f found = intersect(ray, node.children[i]);
			if (found.y != std::numeric_limits<float>::infinity()) {
				return found;
			}
		}
	}
	else {
		bool hit = node.box.intersect(ray, 0, 10000);
		if (hit) {
			ofVec3f vert = mesh.getVertex(node.points[0]);
			return vert;
		}
	}
	return ofVec3f(0, std::numeric_limits<float>::infinity(), 0);
